...
const FEATURES = [
  ...
  },
  {
    ...
    icon: (
      <>
        <path d="M12 3a6 6 0 0 0-6 6c0 2 1 3 1 5v3h10v-3c0-2 1-3 1-5a6 6 0 0 0-6-6Z" stroke="currentColor" strokeWidth="1.4" />
    ...
    <div className="animate-fade-in relative min-h-screen bg-ink">
      ...
      <header className="relative mx-auto flex max-w-[1200px] items-center justify-between px-6 py-5">
        ...
        <nav className="hidden items-center gap-0.5 md:flex">
          {NAV.map((item) => (
            <a key={item} href="#" className="rounded-md px-3.5 py-1.5 text-sm text-muted transition-colors duration-150 hover:bg-panel hover:text-fg">
              {item}
            </a>
          ))}
        </nav>
        <div className="flex items-center gap-1.5">
          <button onClick={() => navigate('/login')} className="rounded-md px-3.5 py-1.5 text-sm font-medium text-muted transition-colors hover:text-fg">
            Log in
          </button>
          <button onClick={() => navigate('/signup')} className="rounded-lg bg-teal px-4 py-1.5 text-sm font-semibold text-ink transition-all duration-200 hover:bg-[#5eead4] hover:shadow-[0_0_20px_-4px_rgba(45,212,191,0.55)] active:scale-[0.98]">
            Get Started
          </button>
        </div>
      </header>
      ...
      <main className="relative mx-auto max-w-[1200px] px-6">
        <div className="grid items-center gap-10 pt-10 lg:grid-cols-[0.9fr_1.1fr] lg:pt-14">
          <div className="animate-fade-up">
            <div className="inline-flex items-center gap-2 rounded-full border border-line-2 bg-panel px-3 py-1">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-teal" style={{ animation: 'blink 1.8s ease-in-out infinite' }} />
              <span className="mono text-[10px] uppercase tracking-widest text-muted">Now in public beta</span>
            </div>
            ...
            <div className="mt-8 flex items-center gap-3">
              <PrimaryButton onClick={() => navigate('/signup')} className="px-6">
                Get Started
              ...
              <GhostButton onClick={() => navigate('/dashboard')} className="px-6">View Demo</GhostButton>
            </div>
          </div>

          <div className="animate-fade-up relative h-[430px] overflow-hidden rounded-2xl border border-line bg-panel/60 lg:h-[490px]" style={{ animationDelay: '0.08s' }}>
            <div className="flex items-center justify-between border-b border-line px-4 py-2.5">
              <span className="mono text-[10px] uppercase tracking-widest text-muted">knowledge_graph.view</span>
        ...
        <div className="relative z-10 mt-14 grid gap-4 sm:grid-cols-3">
          {FEATURES.map((feature, index) => (
            <div key={feature.title} className="animate-fade-up group rounded-xl border border-line bg-panel p-5 transition-all duration-300 hover:border-teal/35 hover:bg-panel-2" style={{ animationDelay: `${0.12 + index * 0.07}s` }}>
              <div className="flex items-center justify-between">
                ...
                <span className="mono text-[10px] tracking-widest text-faint">{feature.tag}</span>
              </div>
              <h3 className="mt-4 text-[14px] font-semibold tracking-tight text-fg">{feature.title}</h3>
              <p className="mt-1.5 text-[13px] leading-relaxed text-muted">{feature.desc}</p>
            </div>
          ))}
        </div>

        <footer className="mt-2 flex items-center justify-between py-8">
          <Logo />
          <span className="mono text-[10px] tracking-wide text-faint">© 2026 MemoryMap · all systems operational</span>
        </footer>
      </main>
    </div>
  );
