    ...
    <div className="grid min-h-screen grid-cols-1 lg:grid-cols-[1.05fr_0.95fr]">
      ...
      <div className="relative hidden flex-col justify-between overflow-hidden border-r border-line bg-panel p-10 lg:flex">
        ...
        <div className="relative max-w-xs">
          <p className="mono text-[10px] uppercase tracking-widest text-teal">
            {isLogin ? "// resume session" : "// map your knowledge"}
          </p>
          <h2 className="mt-3 text-2xl font-semibold leading-snug tracking-tight text-fg">
            {isLogin
              ? "Your knowledge, connected and always in reach."
              : "Turn scattered information into a living knowledge graph."}
          </h2>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center bg-ink px-6 py-10 sm:px-10">
        <div key={mode} className="animate-fade-up w-full max-w-[360px]">
          ...
          <p className="mt-1.5 text-sm text-muted">
            {isLogin
              ? "Sign in to continue."
              : "Start mapping your knowledge in minutes."}
          </p>
              ...
              >
                ...
                <Field
                  ...
                  right={
                    <button
                      type="button"
                      className="mono text-[10px] tracking-wide text-teal transition-colors hover:text-[#5eead4]"
                    >
                      Forgot password?
                    </button>
                  }
                    ...
                    <div className="flex gap-1.5">
                      {[1, 2, 3, 4].map((i) => (
                        <span
                          ...
                          style={{
                            background:
                              i <= s ? STRENGTH_META[s].color : "#1c2429",
                          }}
                        />
                      ))}
                    </div>
                    <p
                      className="mono mt-1.5 text-[10px] tracking-wide"
                      style={{ color: STRENGTH_META[s].color || "#56636a" }}
                    >
                      {s > 0
                        ? `Password strength · ${STRENGTH_META[s].label}`
                        : ""}
                    </p>
                  </div>
                )}
          ...
          <p className="mt-6 text-center text-xs text-muted">
            ...
            <button
              onClick={onSwitch}
              className="font-medium text-teal transition-colors hover:text-[#5eead4]"
            >
